...
def stream_label(stream: dict[str, Any], audio_number: int) -> str:
    ...
    title = tags.get("title") or tags.get("handler_name") or "sans titre"
    language = tags.get("language", "langue inconnue")
    return f"piste {audio_number} | {language} | {title}"
...
def choose_audio_stream(streams: list[dict[str, Any]], audio_index: int | None, language: str | None, title: str | None) -> tuple[int, int]:
    ...
    if audio_index is not None:
        if audio_index < 0 or audio_index >= len(audio_streams):
            raise ValueError(f"--audio-index doit être compris entre 0 et {len(audio_streams) - 1}")
        ...
        if not candidates:
            raise ValueError("aucune piste audio ne correspond aux filtres demandés")
...
def list_audio_tracks(input_dir: Path, ffprobe: str) -> int:
    ...
    if not files:
        print(f"Aucun fichier média dans {input_dir}")
        return 0
    for source in files:
        print(f"\n{source.name}")
